...
class ChatBoardViewSet(viewsets.ViewSet):
    """Unified chat boards across all features - returns REAL data"""
    permission_classes = [permissions.IsAuthenticated]
    
    def list(self, request):
        user = request.user
        boards = {
            'marketplace': self._get_marketplace_chats(user),
            'tutoring': self._get_tutoring_chats(user),
            'gigs': self._get_gig_chats(user),
            'mesh': self._get_mesh_rooms(user),
        }
        return Response(boards)
    
    def _get_marketplace_chats(self, user):
        from marketplace.models import Order
        chats = []
        orders = Order.objects.filter(
            Q(buyer=user) | Q(product__seller=user)
        ).select_related('product', 'buyer', 'product__seller').order_by('-created_at')[:20]
        
        seen = set()
        for order in orders:
            other_user = order.product.seller if order.buyer == user else order.buyer
            if not other_user or other_user == user:
                continue
            key = f"mp-{min(user.id, other_user.id)}-{max(user.id, other_user.id)}-{order.product.id}"
            if key in seen:
                continue
            seen.add(key)
            chats.append({
                ...
                'other_avatar': getattr(other_user, 'avatar_url', None),
                'product_id': str(order.product.id),
                'product_title': order.product.title,
                'last_message': f'Purchase: {order.product.title}',
                'last_message_time': order.created_at.isoformat(),
                'unread_count': 0,
            })
        return chats
    
    def _get_tutoring_chats(self, user):
        from tutoring.models import TutoringSession, TutoringChatMessage
        chats = []
        sessions = TutoringSession.objects.filter(
            Q(tutor=user) | Q(student=user)
        ).select_related('tutor', 'student').order_by('-scheduled_at')[:20]
        
        for session in sessions:
            other_user = session.student if session.tutor == user else session.tutor
            if not other_user or other_user == user:
                continue
            last_msg = TutoringChatMessage.objects.filter(session=session).order_by('-created_at').first()
            chats.append({
                'id': str(session.id),
                'other_user': other_user.username,
                'other_avatar': getattr(other_user, 'avatar_url', None),
                ...
                'last_message': last_msg.text[:100] if last_msg else f'Tutoring: {session.subject}',
                'last_message_time': (last_msg.created_at if last_msg else session.scheduled_at).isoformat(),
                'unread_count': 0,
            })
        return chats
    
    def _get_gig_chats(self, user):
        from gigs.models import Gig, GigChatMessage
        chats = []
        gigs = Gig.objects.filter(
            Q(creator=user) | Q(taker=user)
        ).select_related('creator', 'taker').order_by('-created_at')[:20]
        
        for gig in gigs:
            other_user = gig.taker if gig.creator == user else gig.creator
            if not other_user or other_user == user:
                continue
            last_msg = GigChatMessage.objects.filter(gig=gig).order_by('-created_at').first()
            chats.append({
                'id': str(gig.id),
                'other_user': other_user.username,
                'other_avatar': getattr(other_user, 'avatar_url', None),
                'gig_id': str(gig.id),
                'gig_title': gig.title,
                'last_message': last_msg.text[:100] if last_msg else f'Gig: {gig.title}',
                'last_message_time': (last_msg.created_at if last_msg else gig.created_at).isoformat(),
                'unread_count': 0,
            })
        return chats
    
    def _get_mesh_rooms(self, user):
        """Get WaveMesh and private chat rooms with real messages"""
        from mesh.models import ChatRoom, ChatRoomMembership, ChatMessage
        chats = []
        
        memberships = ChatRoomMembership.objects.filter(
            user=user
        ).exclude(
            room__name__startswith='marketplace-'
        ).exclude(
            room__name__startswith='gig-'
        ).exclude(
            room__name__startswith='tutoring-'
        ).select_related('room').order_by('-room__created_at')[:20]
        ...
        for membership in memberships:
            ...
            last_msg = ChatMessage.objects.filter(room=room).order_by('-created_at').first()
            
            chats.append({
                'id': str(room.id),
                'other_user': other_user.username if other_user else 'Unknown',
                'other_avatar': getattr(other_user, 'avatar_url', None) if other_user else None,
                'room_id': str(room.id),
                'room_name': getattr(room, 'name', f'Chat with {other_user.username if other_user else "Unknown"}'),
                'last_message': last_msg.content[:100] if last_msg and last_msg.content else ('📷 Photo' if last_msg and last_msg.file_url else 'No messages yet'),
                'last_message_time': (last_msg.created_at if last_msg else room.created_at).isoformat(),
                'unread_count': 0,
            })
        return chats
