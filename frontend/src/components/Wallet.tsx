/**
 * Sasl - Advanced Wallet with complete transaction history and categories
 */
...
import {
  Wallet as WalletIcon, TrendingUp, ArrowDownCircle, Loader2,
  AlertCircle, DollarSign, Heart, ShoppingCart, Star, Video, BookOpen
} from 'lucide-react';
...
export default function Wallet() {
  ...
  const categoryTotals: CategoryTotal[] = Object.entries(
    transactions.reduce((acc, tx) => {
      ...
      acc[type].total += tx.amount > 0 ? tx.amount : 0;
      acc[type].count += 1;
      return acc;
          ...
          {filteredTransactions.map(tx => (
            ...
            >
              ...
              <span className={`font-bold ${tx.amount >= 0 ? 'text-green-600' : 'text-red-500'}`}>
                {tx.amount >= 0 ? '+' : ''}{Number(tx.amount).toFixed(2)} USD
              </span>
            </motion.div>
          ))}
        </div>
