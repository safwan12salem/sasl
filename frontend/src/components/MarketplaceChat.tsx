...
export default function MarketplaceChat({ roomId, onClose }: Props) {
  ...
  useEffect(() => {
    const fetchHistory = async () => {
      try {
        const res = await api.get(`/marketplace/chat/${roomId}/`);
  ...
  const send = async () => {
    ...
    try { await api.post(`/marketplace/chat/${roomId}/`, { text: input }); } catch {}
      ...
      <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl">
        <div className="flex items-center justify-between p-4 border-b">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-bold text-gray-800">{t('Marketplace Chat')}</h3>
          </div>
