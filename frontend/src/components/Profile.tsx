      ...
      <div className="max-w-4xl mx-auto px-4 -mt-16 relative z-10">
              ...
              <div>
                {isEditing ? (
                  ...
                  <h1 className="text-2xl font-bold text-gray-900">
                    ...
                    {profile.is_verified && <span className="ml-2 inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800"><Check size={12} className="mr-1" /> {t('Verified')}</span>}
                  </h1>
                )}
        ...
        <div className="flex border-b border-gray-200 mt-8 overflow-x-auto">
          {[
            ...
            <button key={key} onClick={() => { setActiveTab(key as any); fetchUserContent(key); }}
              className={`flex items-center gap-1 px-6 py-3 text-sm font-semibold transition border-b-2 -mb-px whitespace-nowrap ${activeTab === key ? 'border-green-500 text-green-600' : 'border-transparent text-gray-500 hover:text-gray-700'}`}>
              {icon} {label}
            </button>
          ))}
        </div>
        ...
        <div className="py-6">
          {activeTab === 'posts' && (
            <div className="space-y-3">
              ...
              {userPosts.map((post: any) => <div key={post.id} className="glass p-4 rounded-xl"><p className="text-sm">{post.text}</p><div className="flex gap-4 mt-2 text-xs text-gray-500"><span>❤️ {post.likes_count}</span><span>💬 {post.comments_count}</span></div></div>)}
            </div>
          )}
          {activeTab === 'reels' && (
            <div className="grid grid-cols-2 gap-3">
              {userReels.length === 0 && <p className="text-gray-500 text-center py-10 col-span-2">{t('no_reels_yet')}</p>}
              {userReels.map((reel: any) => <div key={reel.id} className="glass rounded-xl overflow-hidden"><video src={reel.video_url} className="w-full h-32 object-cover" /><p className="p-2 text-xs">{reel.caption}</p></div>)}
            </div>
          )}
          {activeTab === 'products' && (
            <div className="space-y-3">
              {userProducts.length === 0 && <p className="text-gray-500 text-center py-10">{t('no_products')}</p>}
              {userProducts.map((p: any) => <div key={p.id} className="glass p-3 rounded-xl flex gap-3">{p.image_url && <img src={p.image_url} className="w-16 h-16 rounded-lg object-cover" alt="" />}<div><p className="font-semibold text-sm">{p.title}</p><p className="text-green-600">${p.price}</p></div></div>)}
            </div>
          )}
          {activeTab === 'gigs' && (
            <div className="space-y-3">
              {userGigs.length === 0 && <p className="text-gray-500 text-center py-10">{t('no_gigs')}</p>}
              {userGigs.map((g: any) => <div key={g.id} className="glass p-3 rounded-xl"><p className="font-semibold text-sm">{g.title}</p><p className="text-xs text-gray-500">{g.status} · ${g.budget}</p></div>)}
            </div>
          )}
          {activeTab === 'portfolio' && (
            <div>
              <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                {portfolio.map(item => (
                  <motion.div key={item.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="glass p-3 rounded-xl">
                    {getFullImageUrl(item.image_url) && <img src={getFullImageUrl(item.image_url)!} className="w-full h-32 object-cover rounded-lg mb-2" alt="" />}
                ...
                {isOwnProfile && (
                  <button onClick={() => setShowPortfolioForm(true)} className="glass p-3 rounded-xl border-2 border-dashed border-gray-300 flex flex-col items-center justify-center text-gray-400 hover:text-green-500 hover:border-green-400 transition min-h-[150px]">
                    <PlusCircle size={32} /><span className="text-sm mt-2 font-semibold">{t('Add Project')}</span>
                  </button>
                )}
              </div>
              <AnimatePresence>
                {showPortfolioForm && (
                  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4" onClick={() => setShowPortfolioForm(false)}>
                    <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} exit={{ scale: 0.9 }} className="bg-white rounded-2xl p-6 max-w-md w-full shadow-2xl" onClick={e => e.stopPropagation()}>
                      ...
                      <div className="mb-3"><label className="btn-ghost cursor-pointer flex items-center gap-1 text-sm"><ImageIcon size={16} /> {pfImage ? pfImage.name : t('Upload Image')}<input type="file" accept="image/*" className="hidden" onChange={e => setPfImage(e.target.files?.[0] || null)} /></label></div>
                      <div className="flex gap-2"><button onClick={addPortfolioItem} className="btn-primary flex-1">{t('Add to Portfolio')}</button><button onClick={() => setShowPortfolioForm(false)} className="btn-ghost">{t('Cancel')}</button></div>
                    </motion.div>
                  </motion.div>
                )}
              </AnimatePresence>
