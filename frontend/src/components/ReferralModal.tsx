  ...
  const shareLink = async () => {
    if (navigator.share) {
      await navigator.share({
        title: 'Join me on Sasl!',
        text: 'Join Sasl - the social network that works offline! Use my referral link and we both get $1!',
        url: referralLink,
      });
    } else {
      copyLink();
      ...
      >
        <div className="flex justify-between items-center mb-4">
          <h3 className="font-bold text-xl flex items-center gap-2">
            <Gift className="text-yellow-500" /> Invite Friends
          </h3>
          <button onClick={onClose}><X size={20} /></button>
        </div>
        
        <p className="text-gray-500 mb-4">
          Share Sasl with friends and you both earn <strong>$1.00</strong> when they join!
        </p>
