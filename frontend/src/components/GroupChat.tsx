...
const handleEditMessage = async (messageId: string, oldText: string) => {
  ...
  try {
    await api.post(`/groupchat/groups/${activeGroup}/edit_message/`, {
...
const handleDeleteMessage = async (messageId: string) => {
  ...
  try {
    await api.post(`/groupchat/groups/${activeGroup}/delete_message/`, {
      message_id: messageId,
      for_everyone: true,
    });
    fetchMessages(activeGroup!, true);
...
useEffect(() => {
  if (activeGroup) {
    localStorage.setItem('sasl_active_group', activeGroup);
  }
}, [activeGroup]);


// Restore active group when groups load
useEffect(() => {
  const savedGroupId = localStorage.getItem('sasl_active_group');
  if (savedGroupId && groups.length > 0) {
    const found = groups.find(g => g.id === savedGroupId);
    if (found) {
      setActiveGroup(savedGroupId);
      fetchMessages(savedGroupId);
    }
  }
}, [groups]);
...
<button onClick={() => setShowCreate(prev => !prev)} onTouchEnd={(e) => { e.preventDefault(); setShowCreate(prev => !prev); }} className="p-1.5 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg text-green-600" title={t('new_group')}>
  ...
  msg.sender?.avatar_url ? (
    <img src={msg.sender.avatar_url.startsWith('http') ? msg.sender.avatar_url : `http://localhost:8000${msg.sender.avatar_url}`} 
      className="w-8 h-8 rounded-full object-cover mr-2 flex-shrink-0" alt="" />
  ) : (
    ...
    {isMe && (
      <div className="absolute -top-2 right-0 opacity-0 group-hover:opacity-100 transition flex gap-1">
        <button
          ...
          className="p-1 rounded-full bg-white shadow text-gray-500 hover:text-blue-500 text-xs"
        >
          ✏️
        </button>
        <button
          onClick={() => handleDeleteMessage(msg.id)}
          className="p-1 rounded-full bg-white shadow text-gray-500 hover:text-red-500 text-xs"
        >
          🗑️
        </button>
      </div>
...
{/* Voice message */}
{msg.audio && (
  <div className={`px-4 py-2 rounded-2xl ${isMe ? 'bg-green-500 text-white' : 'bg-white dark:bg-gray-800'}`}>
    <audio controls src={msg.audio} className="max-w-full h-8" />
{(msg.audio_duration ?? 0) > 0 && (
  <span className="text-xs opacity-70">{msg.audio_duration}s</span>
)}
...
>
  <Mic size={18} />
               ...
               <button onClick={() => setShowEmoji(!showEmoji)} className="p-2 rounded-full hover:bg-gray-100 dark:hover:bg-gray-700 text-gray-500">
  <Smile size={18} />
</button>
{showEmoji && (
    ...
    <div className="px-4 py-2 border-t border-gray-200 dark:border-gray-700">
      <VoiceMessageRecorder
        onSend={async (blob, duration) => {
          ...
          try {
            await api.post(`/groupchat/groups/${activeGroup}/send_voice/`, formData, {
              headers: { 'Content-Type': 'multipart/form-data' },
            });
