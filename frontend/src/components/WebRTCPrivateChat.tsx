...
export default function WebRTCPrivateChat({ roomId, onClose }: Props) {
  ...
  useEffect(() => {
    ...
    else if (roomId?.startsWith('marketplace-')) setChatType('gig');
}, [roomId]);
  // Fetch chat history
  useEffect(() => {
    const fetchHistory = async () => {
      try {
        const res = await api.get(`/mesh/rooms/${roomId}/unified-chat/`);
        ...
        const historyMsgs = dataArray.map((m: any) => {
          ...
          return sender === user?.username ? `Me: ${m.content}` : `${sender}: ${m.content}`;
        });
  ...
  useEffect(() => {
   ...
   const wsRoomId = roomId?.startsWith('marketplace-') || roomId?.startsWith('gig-') || roomId?.startsWith('tutoring-') 
  ? roomId 
  : `private_${roomId}`;
const wsUrl = isLocal
  ...
  const send = async () => {
    ...
    // Persist to backend
      try {
      await api.post(`/mesh/rooms/${roomId}/unified-chat/`, { text: input });
    } catch {}
