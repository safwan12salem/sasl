...
import { QrCode, Radio, WifiOff, Shield, Send, LogOut, Copy, Menu, X, ArrowLeft, MessageCircle, Link, Smile, Bluetooth } from 'lucide-react';
...
interface ChatMessage {
  id: string; from: string; text: string; timestamp: number; isMe: boolean;
}
interface ChatRoom {
...
export default function WaveMesh() {
  ...
  useEffect(() => {
    ...
    waveMeshCore.setOnPeerDiscovered((p: any) => {
      setPeers(prev => prev.find(x => x.id === p.id) ? prev : [...prev, p]);
    });

    waveMeshCore.setOnPeerConnected((data: any) => {
      const room: ChatRoom = { id: data.peerId, name: data.username || 'Peer', lastMessage: 'Connected via BLE', unread: 0 };
      setRooms(prev => {
        const ex = prev.find(r => r.id === room.id);
        if (ex) return prev.map(r => r.id === room.id ? { ...r, name: room.name } : r);
        return [room, ...prev];
      });
      setActiveRoom(room);
        ...
        {(showSidebar || window.innerWidth >= 1024) && (
          <motion.div initial={{ x: -320 }} animate={{ x: 0 }} exit={{ x: -320 }}
            ...
            <div className="p-5 border-b border-gray-100 dark:border-gray-800">
              ...
              <p className="text-xs text-gray-500 flex items-center gap-1"><span className="w-2 h-2 bg-green-500 rounded-full animate-pulse" />{status}</p>
              <button onClick={toggleScan} className={`mt-2 w-full py-2 rounded-xl text-xs font-semibold ${scanning ? 'bg-red-500 text-white' : 'bg-blue-500 text-white'}`}>
            ...
            <div className="flex mx-4 mt-3 bg-gray-100/80 dark:bg-gray-800/80 rounded-2xl p-1">
              {[['rooms','Chats',MessageCircle],['discover','Nearby',Bluetooth],['connect','Connect',Link]].map(([k,l,Icon]) => (
                <button key={k as string} onClick={() => setTab(k as any)} className={`flex-1...`}>
  <Icon size={16}/><span>{l as string}</span>
</button>
              ))}
            </div>
            <div className="flex-1 overflow-y-auto mt-2 px-3 pb-3">
              {tab === 'rooms' && (rooms.length === 0 ? <div className="text-center py-16 text-gray-400"><MessageCircle size={36} className="mx-auto mb-2 opacity-50"/><p className="font-semibold">No conversations</p></div> :
                rooms.map(r => <button key={r.id} onClick={()=>{setActiveRoom(r);setShowSidebar(false)}} className={`w-full flex items-center gap-3 p-3.5 rounded-2xl text-left ${activeRoom?.id===r.id?'bg-green-50 dark:bg-green-900/20 border-2 border-green-200':'hover:bg-gray-50 dark:hover:bg-gray-800/50 border-2 border-transparent'}`}><Avatar name={r.name}/><div className="flex-1 min-w-0"><p className="font-semibold text-sm truncate">@{r.name}</p><p className="text-xs text-gray-500 truncate">{r.lastMessage}</p></div></button>)
              )}
              {tab === 'discover' && (peers.length === 0 ? <div className="text-center py-16"><Bluetooth size={36} className="mx-auto mb-3 text-gray-300"/><p className="font-semibold text-gray-500">{scanning ? 'Scanning for Sasl users...' : 'Press Start BLE Scan'}</p><p className="text-sm text-gray-400">BLE Range: 100m</p></div> :
                peers.map(p => <div key={p.id} className="flex items-center gap-3 p-3.5 rounded-2xl hover:bg-gray-50 dark:hover:bg-gray-800/50"><Avatar name={p.username}/><div className="flex-1 min-w-0"><p className="font-semibold text-sm">@{p.username}</p><p className="text-xs text-green-500">🟢 BLE · ~{p.distance}m</p></div><button onClick={() => waveMeshCore.connectToPeer(p.id)} className="p-2.5 bg-green-500 text-white rounded-xl"><Send size={14}/></button></div>)
              )}
              {tab === 'connect' && (
            ...
            <div className="p-4 border-t border-gray-100 dark:border-gray-800 text-center"><p className="text-[10px] text-gray-400">🌊 WaveMesh BLE P2P · 100m Range</p></div>
          </motion.div>
        )}
      </AnimatePresence>
        ...
        {!activeRoom ? (
          <div className="flex-1 flex items-center justify-center p-4 sm:p-8">
            <div className="text-center max-w-md">
              ...
              <p className="text-gray-500 mb-1 sm:mb-2 text-sm sm:text-lg"><span className="font-semibold text-green-600">BLE P2P</span> · No internet needed</p>
              <p className="text-gray-400 text-xs sm:text-sm mb-4 sm:mb-8">100m range via Bluetooth Low Energy</p>
          ...
          <div className="flex-1 flex flex-col min-h-0">
            <div className="px-3 sm:px-4 py-2 sm:py-3 border-b border-gray-200/50 dark:border-gray-800/50 bg-white/80 dark:bg-gray-900/80 backdrop-blur-xl flex items-center justify-between flex-shrink-0">
              <div className="flex items-center gap-2 sm:gap-3 min-w-0 flex-1">
                ...
                <div className="min-w-0"><h3 className="font-bold text-xs sm:text-sm truncate">@{activeRoom.name}</h3><p className="text-[10px] sm:text-xs text-green-600">BLE P2P Connected</p></div>
              </div>
            ...
            <div className="flex-1 overflow-y-auto px-3 sm:px-4 py-3 sm:py-4 space-y-3 min-h-0">
              {messages.length===0 && <div className="text-center text-gray-400 py-16"><MessageCircle size={36} className="mx-auto mb-2 opacity-30"/><p>Connected via BLE P2P</p><p className="text-sm">Say hello! 👋</p></div>}
            ...
            <div className="p-2 sm:p-4 border-t border-gray-200/50 dark:border-gray-800/50 bg-white/80 dark:bg-gray-900/80 backdrop-blur-xl flex-shrink-0">
              <div className="flex items-end gap-1.5 sm:gap-2">
                ...
                <input ref={inputRef} value={input} onChange={e=>setInput(e.target.value)} onKeyDown={e=>{if(e.key==='Enter'){e.preventDefault();sendMsg()}}} placeholder="Message via BLE..." className="flex-1 min-w-0 px-3 sm:px-5 py-2 sm:py-3 rounded-2xl bg-gray-100 dark:bg-gray-800 text-xs sm:text-sm outline-none focus:ring-2 focus:ring-green-400/50"/>
