      ...
      <div className="flex-1 flex flex-col z-10 min-w-0">
        {!activeRoom ? (
          ...
          <div className="flex-1 flex items-center justify-center p-4 sm:p-8">
            <div className="text-center max-w-md w-full">
              <motion.div animate={{ y: [0, -10, 0] }} transition={{ duration: 3, repeat: Infinity }}
                ...
                <Zap size={36} className="text-white sm:size-14" />
              </motion.div>
              <h2 className="text-xl sm:text-3xl font-bold bg-gradient-to-r from-green-600 to-emerald-600 bg-clip-text text-transparent mb-2 sm:mb-3">Welcome to WaveMesh</h2>
              <p className="text-gray-500 mb-1 sm:mb-2 text-sm sm:text-lg">The world's first <span className="font-semibold text-green-600">offline P2P</span> chat network</p>
              <p className="text-gray-400 text-xs sm:text-sm mb-4 sm:mb-8 px-2">Connect directly with anyone nearby — no internet required.</p>
              ...
              <div className="flex gap-2 sm:gap-3 justify-center flex-wrap px-2">
                ...
                <button onClick={generateQRCode}
                  ...
                  <QrCode size={16} className="sm:size-18" /> 🌊 Offline P2P
                </button>
              </div>
              
              <div className="mt-6 sm:mt-8 flex items-center justify-center gap-3 sm:gap-6 text-[10px] sm:text-xs text-gray-400 flex-wrap">
                <span className="flex items-center gap-1"><WifiOff size={10} className="sm:size-12" /> No Internet</span>
                <span className="flex items-center gap-1"><Shield size={10} className="sm:size-12" /> E2E Encrypted</span>
                <span className="flex items-center gap-1"><Zap size={10} className="sm:size-12" /> Instant</span>
              </div>
            </div>
          </div>
        ) : (
          ...
          <div className="flex-1 flex flex-col min-h-0">
            ...
            <div className="px-3 sm:px-4 py-2 sm:py-3 border-b border-gray-200/50 dark:border-gray-800/50 bg-white/80 dark:bg-gray-900/80 backdrop-blur-xl flex items-center justify-between flex-shrink-0">
              <div className="flex items-center gap-2 sm:gap-3 min-w-0 flex-1">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-[10px] sm:text-xs text-green-600">Connected via WaveMesh</p>
                </div>
              </div>
            ...
            {/* Messages */}
            <div className="flex-1 overflow-y-auto px-3 sm:px-4 py-3 sm:py-4 space-y-4 sm:space-y-6 min-h-0">
              {messages.length === 0 && (
                <div className="text-center text-gray-400 py-16 sm:py-20">
                  <MessageCircle size={36} className="mx-auto mb-2 opacity-30 sm:size-48" />
              ...
              {groupedMessages.map((group) => (
                <div key={group.date}>
                  <div className="flex justify-center mb-3 sm:mb-4">
                    <span className="bg-gray-200/50 dark:bg-gray-800/50 text-gray-500 text-[10px] sm:text-xs px-3 sm:px-4 py-0.5 sm:py-1 rounded-full">{group.date}</span>
                  </div>
                  {group.messages.map((msg) => {
                    const isMe = msg.sender.username === myUsername;
                    return (
                      <motion.div key={msg.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
                        className={`flex items-end gap-1.5 sm:gap-2 mb-1 ${isMe ? 'justify-end' : 'justify-start'} mt-2 sm:mt-3`}>
                        ...
                        <div className={`max-w-[80%] sm:max-w-[75%] px-3 sm:px-4 py-2 sm:py-2.5 rounded-2xl text-xs sm:text-sm ${
                          ...
                          {!isMe && <p className="text-[9px] sm:text-[10px] font-semibold text-gray-500 mb-0.5">@{msg.sender.username}</p>}
                          {msg.type === 'image' && msg.fileUrl ? (
                            <img src={msg.fileUrl} alt="Shared" className="max-w-full rounded-xl max-h-48 sm:max-h-64 object-cover" />
            ...
            <div className="p-2 sm:p-4 border-t border-gray-200/50 dark:border-gray-800/50 bg-white/80 dark:bg-gray-900/80 backdrop-blur-xl flex-shrink-0">
              <div className="flex items-end gap-1.5 sm:gap-2">
                ...
                <button onClick={() => setShowEmojiPicker(!showEmojiPicker)} className="p-2 sm:p-3 rounded-2xl bg-gray-100 dark:bg-gray-800 text-gray-500 flex-shrink-0">
                ...
                <button onClick={() => fileInputRef.current?.click()} className="p-2 sm:p-3 rounded-2xl bg-gray-100 dark:bg-gray-800 text-gray-500 flex-shrink-0">
                  <Paperclip size={18} className="sm:size-20" />
                </button>
                  ...
                  <div className="grid grid-cols-7 sm:grid-cols-8 gap-1 sm:gap-1.5">
                    {EMOJIS.map(emoji => (
                      <button key={emoji} onClick={() => { setInput(prev => prev + emoji); setShowEmojiPicker(false); inputRef.current?.focus(); }}
                        className="w-7 h-7 sm:w-8 sm:h-8 flex items-center justify-center hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg text-sm sm:text-lg transition transform hover:scale-125">{emoji}</button>
                    ))}
                  </div>
                </div>
              )}
